...
func (m *Metrics) GetMetrics() error {
	...
	err = m.GetNodeMetrics(mc, clientset)
	if err != nil {
		return err
	}
	return m.GetPodMetrics(mc, clientset)
}
...
func (m *Metrics) GetNodeMetrics(mc *metrics.Clientset, clientset *kubernetes.Clientset) error {
	...
	for _, nodeMetrics := range metrics.Items {
		node := findNode(nodes, nodeMetrics.Name)
		if node != nil {
			m.Nodes = append(m.Nodes, NodeMetrics{
				Name:        node.Name,
				MachineType: node.Labels["beta.kubernetes.io/instance-type"],
				Region:      node.Labels["topology.kubernetes.io/region"],
				CPUUsage:    calcPercentageNodeCPU(node, &nodeMetrics),
				MemoryUsage: calcPercentageNodeMem(node, &nodeMetrics),
			})
		}
	}
	return nil
...
func (m *Metrics) GetPodMetrics(mc *metrics.Clientset, clientset *kubernetes.Clientset) error {
	...
	for _, podMetrics := range metrics.Items {
		pod := findPod(pods, podMetrics.Name)
		if pod != nil {
			m.Pods = append(m.Pods, PodMetrics{
				Name:        pod.Name,
				Node:        pod.Spec.NodeName,
				CPUUsage:    calcPercentagePodCPU(pod, &podMetrics),
				MemoryUsage: calcPercentagePodMem(pod, &podMetrics),
			})
		}
	}
	return nil
...
func calcPercentagePodCPU(pod *v1.Pod, metrics *v1beta1.PodMetrics) float64 {
	total := 0.0
...
func calcPercentagePodMem(pod *v1.Pod, metrics *v1beta1.PodMetrics) float64 {
	total := 0.0
	for _, container := range metrics.Containers {
...
func (m *Metrics) PrintNodes() string {
	var result string = "Node\t\t\t\t\t\tMachine Type\tRegion\t\tCPU Usage\tMemory Usage"
...
func (m *Metrics) PrintPods() string {
	var result string = "Pod\t\t\t\t\t\tCPU Usage\tMemory Usage"
